...
import talk from "../lauren-talk.slim.json";

const CATS: { href: string; when: string; title: string; body: string }[] = [
  {
    href: "#t-0",
    when: "00:00 到 02:02",
    title: "AI 是配對寫程式的人",
...
export function S7Lauren() {
  return (
    <>
      <h2 id="s7">
        <span className="no">7</span>Lauren Tan 工作坊：分類與全稿
      </h2>
      <p>
        ...
        <span className="stamp">{talk.n} 段 · {talk.nFix} 處專名修</span>
        修過的句子帶「改」，游標可看轉寫原文。1000 則／月只在她的推文，帶子講
        20 與 600。
      </p>

      <h3 id="s7-cats">
        <span className="no">7.0</span>依時間切的八塊
      </h3>
      <p>
        同一卷帶子按時間切。點時間跳到全稿對應句。
      ...
      <ol className="catmap">
        {CATS.map((c) => (
          <li key={c.href}>
            <a className="when xref" href={c.href}>
              {c.when}
            </a>
            <span>
              <b>{c.title}</b>
      ...
      <h3 id="s7-full">
        <span className="no">7.1</span>全稿，00:00 到 72:04
      </h3>
      <p>
        {talk.n} 段，模型 {talk.model}。Fable／Sol 在 53:18 改成 Claude／Sonnet。主持在
        70:15 說的 Fable 5 Max 仍照口頭。
      </p>
      <TalkTranscript segments={talk.segments as Seg[]} />
      <p className="src">
        帶子{" "}
        <a className="xref" href="https://x.com/0xCodez/status/2093013117156331552">
          x.com/0xCodez/status/2093013117156331552
